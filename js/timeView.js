...
function constructHeatMap(data, categories, categoryIndex, MaxValue, categoryName) {

    // -- Define Variables -- //
    // Get the container for the heatmap
    const heatmapContainer = document.querySelector('#heatmapContainer' + categoryName);
    ...
    function drawHeatmap() {
        ...
        const heatmapSVG = d3.select('#heatmapContainer' + categoryName)
            .append("svg")
        ...
        const buildYears = () => {
            ...
            for (let i = heatmapMinYear; i <= heatmapMaxYear; i++) {
                array.push(i.toString());

            return array;
            
          };
          const heatmapYears = buildYears();*/
        // Get filtered types
        ...
        const buildTypes = () => {
          ...
          const heatmapTypes = buildTypes();
          
        console.log(heatmapYears)*/
        ...
        const mousemove = function (event, d) {
            ...
            serotypeTip.html("Serotype <b id='heatmapTool_serotypeNum'>" + d.type + "</b>")
        ...
        function highlightYear(year) {
            ...
            //d3.selectAll(".heatmapOverlay").remove();
            // Add the overlay
        ...
        heatmapSVG.selectAll()
            ...
            .style("fill", function (d) {
                const value = d.value;
                const type = d.type;
                if (type === 1) { return colorScales.col1(value); }
                else if (type === 2) { return colorScales.col2(value); }
                else if (type === 3) { return colorScales.col3(value); }
                else if (type === 4) { return colorScales.col4(value); }
            })
...
function calculateAllTimeEvents() {
    ...
    for (year in dataset) {
        for (report in dataset[year]) {
            for (category in allCategoryGroups) {
                if (allCategoryGroups[category].countryList.includes(dataset[year][report].COUNTR)) {
                    // For each type of Dengue Fever
                    allCategoryGroups[category].allTimeEvents[year][1] += dataset[year][report].DEN1;
                    allCategoryGroups[category].allTimeEvents[year][2] += dataset[year][report].DEN2;
                    allCategoryGroups[category].allTimeEvents[year][3] += dataset[year][report].DEN3;
                    allCategoryGroups[category].allTimeEvents[year][4] += dataset[year][report].DEN4;
                }
            }
        }
    };
